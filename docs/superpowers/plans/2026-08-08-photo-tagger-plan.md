...
### Task 5: Список тегов из конфига, а не из кода

**Что и зачем.** Заказчик будет уточнять набор тегов по ходу разметки, и каждый раз править Python ради этого не нужно. Список переезжает в `tools/tags.json` — объект «группа → массив тегов». Встроенный список остаётся запасным вариантом, чтобы скрипт работал даже без конфига рядом.

Ключевое требование к обработке ошибок: конфиг правит человек руками, поэтому опечатка должна давать понятную ошибку на русском и выход, а не молчаливую подмену набора тегов — иначе заказчик решил бы, что размечает своими тегами, а размечал бы встроенными. Отдельно нужен отчёт о тегах, которые есть в уже сделанной разметке, но пропали из конфига: без него правка `tags.json` тихо съедала бы часть проделанной работы при следующем сохранении фотографии.

**Files:**
- Modify: `tools/photo-tagger.py`
- Modify: `tools/test_photo_tagger.py`
- Create: `tools/tags.json`

**Interfaces:**
- Заменяет: константу `TAG_GROUPS` на `DEFAULT_TAG_GROUPS` (запасной список) плюс загрузку из файла. `clean_tags(tags)` получает второй аргумент — `clean_tags(tags, known)`.
- Produces: `parse_tag_groups(data) -> List[Tuple[str, List[str]]]` (валидация, кидает `ValueError` с понятным текстом), `load_tag_groups(path)` (читает файл либо отдаёт встроенный список), `unknown_tags(photos, known) -> Dict[str, int]` (теги из разметки, которых нет в конфиге, и число фотографий по каждому). `make_handler` принимает дополнительный параметр `tag_groups`. Флаг командной строки `--tags`.

- [ ] **Step 1: Написать падающие тесты**

Тесты покрывают: разбор корректного конфига с сохранением порядка; отказ на не-объекте, на группе без списка строк, на дубликате тега в двух группах, на конфиге без единого тега; обрезку пробелов и отбрасывание пустых строк; возврат встроенного списка при отсутствии файла; чтение файла; совпадение поставляемого `tools/tags.json` со встроенным списком; подсчёт неизвестных тегов. Полный текст тестов — в файле `tools/test_photo_tagger.py` (классы `TestTagConfig` и `TestUnknownTagReport`, плюс обновлённые `test_clean_tags_*` с новой сигнатурой).
...
python3 -m unittest discover -s tools -p "test_*.py" -v
```
Ожидается: `AttributeError: module 'photo_tagger' has no attribute 'parse_tag_groups'`.

- [ ] **Step 3: Реализовать**

`parse_tag_groups` валидирует структуру и уникальность тегов; `load_tag_groups` возвращает `DEFAULT_TAG_GROUPS`, если файла нет; `unknown_tags` считает расхождение разметки с конфигом; `clean_tags` принимает список известных тегов вторым аргументом; `make_handler` получает `tag_groups` и вычисляет `known` внутри; `main` разбирает `--tags`, ловит `json.JSONDecodeError` и `ValueError` с понятными сообщениями и печатает предупреждение про пропавшие теги.

- [ ] **Step 4: Создать `tools/tags.json`** с согласованными 21 тегом в четырёх группах, в том же порядке, что и `DEFAULT_TAG_GROUPS` (тест `test_shipped_tags_json_matches_defaults` это проверяет).
...
python3 -m unittest discover -s tools -p "test_*.py" -v
```
Ожидается: 25 тестов проходят.

- [ ] **Step 6: Проверить краевые случаи вживую**

Отдельным конфигом через `--tags`: свой набор тегов отдаётся в `/api/tags` и нормализуется при сохранении; убранный из конфига тег даёт предупреждение при запуске с указанием числа фотографий; сломанный JSON и дубликат тега дают понятную ошибку и выход; отсутствующий файл даёт уведомление и встроенный список. Для запуска сервера в фоне использовать `python3 -u` — иначе вывод буферизуется и не попадёт в лог (`timeout` на macOS отсутствует, останавливать через `pkill`).

- [ ] **Step 7: Commit**

```bash
git add tools/photo-tagger.py tools/test_photo_tagger.py tools/tags.json
git commit -m "feat: tag list from tags.json config"
```
