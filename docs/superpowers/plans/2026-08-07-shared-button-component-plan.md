...
**Архитектура:** `Button.astro` — тонкий `<a class:list={['btn', register, size]}>` с CSS-регистрами по классам, без кастомных свойств (в отличие от `TrustGrid`/`PhotoPlaceholder` — здесь всего 3 фиксированных регистра, а не произвольный цвет, `class:list` проще и понятнее). Побочный эффект: у взрослого регистра впервые появляется hover-состояние (сейчас у кнопок в `AudienceSplit` его нет вовсе) — небольшое, безопасное улучшение к первоначальному запросу «оживить сайт минимальными анимациями», не отдельная задача.
...
- **Визуальный результат на всех трёх существующих страницах (`/`, `/o-studii`, `/detyam`) должен остаться неизменным**, кроме одного намеренного улучшения — hover на кнопках `AudienceSplit` (взрослый регистр), которого раньше не было. Это регрессионный рефакторинг, не редизайн.
